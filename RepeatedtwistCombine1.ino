...
void loop() {
  ...
  switch(state) {
    ...
    case REPEAT_CONFIRM_NEW_PRESET: handleRepeatConfirmNewPreset(key); break;  // new handler
...
bool isRepeatedBendingSubmenu(MenuState st) {
  switch (st) {
    ...
    case REPEAT_CONFIRM_NEW_PRESET:   // updated with new state
...
void showConfirmation(String msg) {
  clearTFT();
  tft.println(msg);
  // Adjusted state handling to properly handle REPEAT_CONFIRM_NEW_PRESET
  if (state == REPEAT_CONFIRM_PRESET_SAVE || state == REPEAT_INPUT_CYCLES || state == REPEAT_EDIT_PRESET || state == REPEAT_CONFIRM_NEW_PRESET) {
    state = REPEATED_BENDING_MENU;
    printRepeatBendingMenu();
  } else if (state == REPEAT_WAITING_EXECUTION) {
    state = REPEATED_BENDING_MENU;
    printRepeatBendingMenu();
  } else if (state == REPEAT_SHOW_RESULT) {
    state = MAIN_MENU;
    printMainMenu();
  } else if (state == TWIST_WAITING_EXECUTION) {
    state = MAIN_MENU;
    printMainMenu();
  } else {
    state = MAIN_MENU;
...
void handleRepeatCycleInput(char key) {
  if (key >= '0' && key <= '9') {
    ...
    if (repeatManualMode) {
      ...
      showRepeatConfirmScreen();  // Call to the confirmation screen
      return;
    }
    ...
    // For adding new preset, transition to confirmation state instead of saving directly
    if (RPCount >= MAX_PRESETS) {
      tft.fillScreen(TFT_RED);
      tft.setTextColor(TFT_WHITE, TFT_RED);
      tft.setCursor(0, 0);
...
void clearRepeatPresets() {
    RPCount = 0;
    // Hapus kunci spesifik untuk repeat presets
    for (int i = 0; i < MAX_PRESETS; i++) { // Hapus semua kemungkinan slot
        preferences.remove(("repeatPreset" + String(i)).c_str());
    }
    preferences.remove("RPCount"); // Jangan lupa hapus count-nya juga
    // Tidak perlu preferences.clear() di sini jika Anda ingin data twist test tetap ada
    saveRepeatPresets(); // Simpan state kosong
}
...
void handleTwistMenu(char key) {
  switch (key) {
    ...
    case '5':
      clearTwistPresets();
      showConfirmation("Semua preset direset!");
      break;
  }
}
...
void handleTwistInputRight(char key) {
  if ((key >= '0' && key <= '9') || key == 'D') {
    ...
    float value = twistInputBuffer.toFloat();
    twistInputBuffer = "";
    if (twistManualMode) twistManualInput.rightCycles = value;
    else twistPresets[twistSelectedIndex].rightCycles = value;
...
void clearTwistPresets() {
    TPCount = 0;
    // Hapus kunci spesifik untuk twist presets
    for (int i = 0; i < MAX_PRESETS; i++) {
        preferences.remove(("left" + String(i)).c_str());
        preferences.remove(("right" + String(i)).c_str());
    }
    preferences.remove("TPCount");
    // Tidak perlu preferences.clear() di sini
    saveTwistPresets(); // Simpan state kosong
}
...
void handleSerialMessage(String msg) {
  if (msg.startsWith("PROG:")) {
    ...
    showResultMessage("Siklus selesai!");
  } else if (msg == "CANCELLED") {
    showResultMessage("Dibatalkan oleh user.");
  }
}
